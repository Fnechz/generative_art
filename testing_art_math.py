...
#exponential class produces relatively boring art
class Exponential:
   ...
   def eval(self, x, y):
      return abs(e ** (1j * self.arg.eval(x, y)))
...
#hacovercosine(x) is a periodic function defined by (1 + sin(x)) / 2
class Hacovercosine:
   def __init__(self, prob):
      self.arg = buildExpr(prob * prob)

   def __str__(self):
      return "hacoversin(pi*" + str(self.arg) + ")"

   def eval(self, x, y):
      return (1 + math.sin(math.pi * self.arg.eval(x, y))) / 2
...
class Haversine:
   ...
   def __str__(self):
      return "haversin(pi*" + str(self.arg) + ")"

   def eval(self, x, y):
      return (1 - math.cos(math.pi * self.arg.eval(x, y))) / 2      
...
class ScaledSin:
   ...
   def eval(self, x, y):
      return abs(math.sin(2 * math.pi * self.arg.eval(x, y)/ 2.5))     
...
def buildExpr(prob=0.99):
   if random.random() < prob:
      return random.choice([Exponential, ScaledSin, Haversine, Hacovercosine, TanPi, Times])(prob)
   else:
...
def makeImage(numPics=10):
    with open("equations.txt", 'w') as eqnsFile:
        ...
        for i in range(numPics):
            ...
            image.save("img" + str(i + 9) + ".png", "PNG")
